  ...
  exit();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (isset($_POST['approve_id'])) {
        ...
        $fetch_sql = "SELECT expense_categories, mode_of_payment, amount FROM cash WHERE id = ?";
        ...
        $stmt_fetch->close();
         ...
         if ($expense_category === 'Account Payable') {
           ...
           $update_journal_sql = "UPDATE ledger_table 
                                ...
                                WHERE expense_categories = 'Cash' AND credit_account = 'Assets'";
           $stmt_update_journal = $conn->prepare($update_journal_sql);
           $stmt_update_journal->bind_param("d", $amount);
           $stmt_update_journal->execute();
         } else {
          ...
          // Insert a new credit entry for 'Cash' under Assets
          $insert_journal_sql = "INSERT INTO ledger_table (expense_categories, debit_amount, credit_amount, credit_account, transaction_date)
                                 VALUES ('Cash', 0, ?, 'Assets', NOW())";
          $stmt_insert_journal = $conn->prepare($insert_journal_sql);
          $stmt_insert_journal->bind_param("d", $amount);
          $stmt_insert_journal->execute();
        } else {
        ...
        if ($source_type === 'budget') {
          ...
          $update_alloc_sql = "UPDATE budget_allocations 
              SET amount_spent = amount_spent + ?, 
                  remaining_balance = remaining_balance - ? 
              WHERE department = ? AND expense_categories = ?";
          $stmt_alloc = $conn->prepare($update_alloc_sql);
          $stmt_alloc->bind_param("ddss", $amount, $amount, $requested_department, $expense_category);
          $stmt_alloc->execute();

          // Insert ledger entries for audit trail
          $insert_ledger_sql = "INSERT INTO ledger_table (expense_categories, debit_amount, credit_amount, credit_account, transaction_date)
                                VALUES (?, ?, 0, 'Expense', NOW())";
          $stmt_ledger = $conn->prepare($insert_ledger_sql);
          $stmt_ledger->bind_param("sd", $expense_category, $amount);
          $stmt_ledger->execute();

          $insert_ledger_sql = "INSERT INTO ledger_table (expense_categories, debit_amount, credit_amount, credit_account, transaction_date)
                                VALUES ('Cash', 0, ?, 'Assets', NOW())";
          $stmt_ledger = $conn->prepare($insert_ledger_sql);
          $stmt_ledger->bind_param("d", $amount);
          $stmt_ledger->execute();
        }

    // Insert record into dr table from cash with modified reference_id
    $insert_sql = "INSERT INTO dr (id, account_name, requested_department, expense_categories, amount, description, document, payment_due, mode_of_payment, reference_id)
      SELECT id, account_name, requested_department, expense_categories, amount, description, document, payment_due, mode_of_payment, 
    ...
    if ($conn->query($insert_sql) === TRUE) {
      ...
      if ($conn->query($update_tr_sql) === TRUE) {
          ...
          // After successful insertion, delete the record from cash
    ...
    // Fetch the filtered and paginated records from cash table
      ...
      <div class="w-full">
        <div>
          <div class="flex items-center justify-between">
            <form method="GET" action="cash.php" class="flex flex-wrap items-center gap-4 mb-4">
          ...
          <div class="bg-white rounded-xl shadow-md border border-gray-200 p-4 mb-6">
            <div class="flex items-center mt-4 mb-4 mx-4 space-x-3 text-purple-700">
              ...
              <h2 class="text-2xl font-poppins text-black">Cash Payout</h2>
            </div>
            <!-- TABLE -->
            <div class="overflow-x-auto w-full">
              <table class="w-full table-auto bg-white mt-4" id="cashTable">
                <thead>
...
let table = document.querySelector("#cashTable");
