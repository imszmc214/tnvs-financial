    ...
    if ($type == 'budget') {
        $stmt = $conn->prepare("
            INSERT INTO budget_request (
                reference_id, account_name, requested_department, mode_of_payment, expense_categories, amount, description, document, time_period, payment_due, bank_name, bank_account_name, bank_account_number, ecash_provider, ecash_account_name, ecash_account_number, status, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param(
            "sssssdssssssssssss",
            ...
            $extra_fields['ecash_provider'], $extra_fields['ecash_account_name'], $extra_fields['ecash_account_number'], $status, $created_at
        );
    } elseif ($type == 'petty_cash') {
        $stmt = $conn->prepare("
            INSERT INTO pettycash (
                reference_id, account_name, requested_department, mode_of_payment, expense_categories, amount, description, document, time_period, payment_due, bank_name, bank_account_name, bank_account_number, ecash_provider, ecash_account_name, ecash_account_number, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param(
            "sssssdsssssssssss",
            $reference_id, $account_name, $department, $mode_of_payment, $category, $amount, $description, $document,
            $time_period, $payment_due, $extra_fields['bank_name'], $extra_fields['bank_account_name'], $extra_fields['bank_account_number'],
            $extra_fields['ecash_provider'], $extra_fields['ecash_account_name'], $extra_fields['ecash_account_number'], $created_at
        );
    } elseif ($type == 'payable') {
        $amount_paid = 0;
        $approval_date = null;
        ...
        $stmt->bind_param(
            ...
            $extra_fields['ecash_account_name'], $extra_fields['ecash_account_number']
        );
        ...
        $stmt = $conn->prepare("
            INSERT INTO pa (
                ...
                ecash_provider, ecash_account_name, ecash_account_number, from_payable
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $from_payable = 1;
        $stmt->bind_param(
            ...
            $extra_fields['ecash_provider'], $extra_fields['ecash_account_name'], $extra_fields['ecash_account_number'], $from_payable
        );
    } else {
...
$displayType = $typeLabels[$type] ?? ucfirst($type);
    ...
    if ($stmt->execute()) {
        $notifMsg = "New {$type} request submitted.<br>Reference ID: $reference_id";
        $notifStmt = $conn->prepare("INSERT INTO notifications (message, request_type) VALUES (?, ?)");
        $notifStmt->bind_param("ss", $notifMsg, $displayType);
        $notifStmt->execute();
        ...
        <main class="flex-1 px-10 py-8 overflow-y-auto">
            ...
            <div class="flex justify-between items-center px-6 pb-6 -mt-2 font-poppins">
                ...
                <div class="text-sm">
                    ...
                    /
                    <a href="request_portal.php" class="text-blue-600 hover:text-blue-600">Financial Request Portal</a>
                </div>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-xl p-8 w-[450px] relative fade-in overflow-y-auto max-h-[90vh]">
                    ...
                    <form id="dynamicRequestForm" enctype="multipart/form-data" autocomplete="off">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="account_name" id="account_name" class="w-full px-3 py-2 border rounded-lg" value="<?= htmlspecialchars($requestor_name) ?>" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="block text-sm font-semibold mb-1">Requesting Department</label>
                            <select name="department" id="department" class="w-full px-3 py-2 border rounded-lg" required>
...
function showForm() {
    ...
    document.getElementById('account_name').value = requestorName;
    // Show/hide time period
    document.getElementById('timePeriodGroup').style.display = (selectedType == 'budget') ? '' : 'none';
...
function loadNotifications() {
    ...
    setTimeout(loadNotifications, 10000); // refresh every 10s
}
</script>
</body>
</html>