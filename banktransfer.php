...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['approve_id'])) {
    ...
    $fetch_sql = "SELECT expense_categories, mode_of_payment, amount FROM bank WHERE id = ?";
    ...
    if ($expense_category === 'Account Payable') {
      ...
      $update_journal_sql = "UPDATE ledger_table 
                           ...
                           WHERE expense_categories = 'Money in Bank' AND credit_account = 'Assets'";
      $stmt_update_journal = $conn->prepare($update_journal_sql);
      $stmt_update_journal->bind_param("d", $amount);
      $stmt_update_journal->execute();
    } else {
      ...
      $stmt_update_journal->execute();
          ...
          // Insert a new credit entry for 'Bank' under Assets
          $insert_journal_sql = "INSERT INTO ledger_table (expense_categories, debit_amount, credit_amount, credit_account, transaction_date)
                                 VALUES ('Bank', 0, ?, 'Assets', NOW())";
          $stmt_insert_journal = $conn->prepare($insert_journal_sql);
          $stmt_insert_journal->bind_param("d", $amount);
          $stmt_insert_journal->execute();
        } else {
    ...
    // Insert record into dr table from bank with modified reference_id
    ...
    if ($conn->query($insert_sql) === TRUE) {
      ...
      if ($conn->query($update_tr_sql) === TRUE) {
        // Delete the record from  bank
        $delete_sql = "DELETE FROM  bank WHERE id = '$approveId'";
        
        if ($conn->query($delete_sql) === TRUE) {
          echo "<div class='bg-green-500 text-white p-4 rounded'>Disbursement Approved, Status Updated in TR and DR!</div>";
        } else {
          echo "<div class='bg-red-500 text-white p-4 rounded'>Error deleting record from bank: " . $conn->error . "</div>";
        }
      } else {
        echo "<div class='bg-red-500 text-white p-4 rounded'>Error updating status in tr: " . $conn->error . "</div>";
      }
    } else {
      echo "<div class='bg-red-500 text-white p-4 rounded'>Error inserting record into dr: " . $conn->error . "</div>";
    }
  }
